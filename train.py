...
def train(train_loader, net, optim, curr_epoch, writer, scheduler, max_iter):
    ...
    for i, data in enumerate(train_loader):
        ...
        for di, ingredients in enumerate(zip(inputs, gts, aux_gts)):
            ...
            # all
            gt = torch.cat((gt, gt, gt, gt), dim=0)
            ...
            # all
            aux_gt = torch.cat((aux_gt, aux_gt, aux_gt, aux_gt), dim=0)

            optim.zero_grad()
            if args.use_isw:
                outputs, loss_diff_texture, main_out = net(input, global_trans=False, iter=curr_iter,
                                                           angular_hallucination=True,
                                                           color_hallucination=True,
                                                           texture_hallucination=True,
                                                           cls_gts=cls_gt, gts=gt, aux_gts=aux_gt, img_gt=img_gt,
            ...
            angular_prob = outputs_sm[B:2*B]
            color_prob = outputs_sm[2 * B:3 * B]
            texture_prob = outputs_sm[3 * B:4 * B]

            loss_consist = 1.0 * kl_categorical(im_prob, angular_prob) + 1.0 * kl_categorical(im_prob, color_prob) + 1.0 * kl_categorical(im_prob, texture_prob)
            ...
            outputs_index += 1
            total_loss = main_loss + (0.4 * aux_loss) + loss_consist
            ...
            loss_diff_texture_value = 0
            loss_consist_value = loss_consist.item()
            ...
            if args.local_rank == 0:
                if i % 50 == 49:
                    ...
                    msg = '[epoch {}], [iter {} / {} : {}], [loss {:0.6f}], [main_loss_value {:0.6f}], ' \
                          '[aux_loss_value {:0.6f}],[loss_consist_value {:0.6f}],[loss_diff_texture_value {:0.6f}],[time {:0.4f}]'.format(
                        curr_epoch, i + 1, len(train_loader), curr_iter, train_total_loss.avg, main_loss_value,
                        aux_loss_value, loss_consist_value, loss_diff_texture_value,
